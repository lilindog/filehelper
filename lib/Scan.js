const { readdirSync, existsSync, statSync, accessSync, constants: { R_OK } } = require("fs");
...
class Scan extends ScanEvent {
    ...
    _scan (
        ...
        try {
            ...
            this.emit(ScanEvent.types.auth, dirNode);
            return;
        }
        ...
        childs.forEach(dirent => {
            if (this._isIgnore(dirent, deep)) return;
    ...
    _isIgnore (dirent, deep) {
        if (this.depth > Scan.defaultOptions.depth && deep > this.depth) return true;
        if (this.excludes.includes(dirent.name)) return true;
        return false;
    }
}
Scan.defaultOptions = {
